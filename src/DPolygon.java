...
//        }
    ...
    void updatePolygon()
    {
        ...
        for(int i = 0; i < x.length; i++)
        {
            newX[i] = dx + Toolkit.getDefaultToolkit().getScreenSize().width/(double)2 + 50 * Calculator.CalculatePositionX(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
            newY[i] = dy + Toolkit.getDefaultToolkit().getScreenSize().height/(double)2 + 50 * Calculator.CalculatePositionY(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
        }
